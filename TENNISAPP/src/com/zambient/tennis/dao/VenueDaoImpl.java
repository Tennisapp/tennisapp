...
public class VenueDaoImpl implements VenueDao{
	...
	public String addVenue(VenueBean venueBean) {
		...
		String QUERY="INSERT INTO `venues` (venue_name,venue_location,venue_address,venue_loc_coordinate,no_of_courts,contact_mob,contact_email,sport_type,venue_light) VALUES(?,?,?,?,?,?,?,?,?)";
		String result=null;
		try{
    		...
    		int val = jdbcTemplate.update(QUERY, new Object[]{venueBean.getVenue_name(),venueBean.getVenue_location(),venueBean.getVenue_address(),venueBean.getVenue_loc_coordinate(),venueBean.getNo_of_courts(),
	...
	public String organizerRegistration(OrganizerRegistartion organizerRegistartion) {
		String QUERY = "INSERT INTO `organizer` (org_name,org_add,org_add2,org_phone,org_mailid,org_password,org_repassword) VALUES(?,?,?,?,?,?,?)";
		...
		try{
    		...
    		if(size <= 0){
			int val = jdbcTemplate.update(QUERY, new Object[]{organizerRegistartion.getName(),organizerRegistartion.getAddress(),organizerRegistartion.getAddress2(),organizerRegistartion.getPhone(),organizerRegistartion.getMailId(),organizerRegistartion.getPassword(),organizerRegistartion.getRepassword()});
    		if(val >0)
    			result = "Success";
    		else
    			result = "failure";
    	   ...
    	   try{
   			...
   			if(rows != null && rows.size()>0) {
   				for(Map row : rows){
   					...
   					venueAndCourtBean.setVenueAddress2(row.get("venue_address")!=null?row.get("venue_address").toString():"");
   					venueAndCourtBean.setCoordinates(row.get("venue_loc_coordinate")!=null?row.get("venue_loc_coordinate").toString():"");
