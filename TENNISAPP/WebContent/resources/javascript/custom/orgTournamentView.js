...
$("#updateTournamentStartDate").change(function(){
	 var tournamentStartDate= $("#updateTournamentStartDate").val();
	  var regEndDateTemp = new Date(tournamentStartDate);
...
var tournamentUrl = url+"playerView/getMyTournamentDetails?tournamentId="+ $('#tournamentid').val();
$http({
	method : "GET",
	url : tournamentUrl
  ...
  $scope.prepareAndSplitDropDown(response.data.categoryList);
	...
	$scope.getTournamentPublishOrNot(mlist.tournamentId);
	$scope.tournamentIdForReg = mlist.tournamentId;
...
$scope.expandeDiv = function(){
	...
	for(var i = 0; i < ($scope.mytournamentdetails.categoryList).length; i++){
		if (category == ($scope.mytournamentdetails.categoryList[i]).categoryName)
        {
			$scope.singleCategory = $scope.mytournamentdetails.categoryList[i];
			$scope.viewPlayersTournament($scope.tournamentCat.tournamentId,$scope.singleCategory.categoryId,$scope.singleCategory.tournamentFee,$scope.singleCategory.playerList,$scope.singleCategory.tournamentFee,$scope.tournamentCat.tournamentStartDate,$scope.singleCategory.matchType,$scope.tournamentCat.title,$scope.singleCategory.categoryName);		
			$scope.CatNotExist ="";
		  $('#hideThis').show();
		  break;
        }else{
...
$scope.getPlayerPublishOrNot = function(tournamentId,categoryId){
	...
	$http({
		...
		url : existingPlayerUrl
		  ...
		  console.log("shiva>>>>"+JSON.parse(response.data));
	  }, function myError(response) {
		//alert(response);
	  });
}

$scope.getTournamentPublishOrNot = function(tournamentId){
	var existingPlayerUrl = url+"playerView/getTournamentPublishMethod?tournamentId="+tournamentId;
	$http({
		method : "GET",
		url : existingPlayerUrl
	  }).then(function mySuccess(response) {
		  $scope.tournamentPublish = JSON.parse(response.data);
		  console.log("shiva>>>>"+JSON.parse(response.data));
	  }, function myError(response) {
		//alert(response);
	  });
...
$scope.activeOrInActiveDrawPublish = function(boolVal){
    ...
    $http({
        ...
        url : transactionUrl
          ...
          console.log("publishDraw>>>" + JSON.stringify(response));
          if($scope.drawPublish == true){
        	  $scope.drawPublish = false;
          }else{
        	  $scope.drawPublish = true;
          }
      }, function myError(response) {
       });
   }
$scope.activeOrInActivePlayersPublish = function(boolVal){
   ...
   $http({
       ...
       url : transactionUrl
    	 ...
    	 $scope.getPlayerPublishOrNot($scope.tournamentIdForReg,$scope.categoryIdForReg);
         
     }, function myError(response) {
      });
  }

$scope.activeOrInActiveTournamentPublish = function(boolVal){
	//alert();
	 var activeVal = 1;
    if(boolVal){
   	 activeVal = 0;
   ...
   var transactionUrl = url+"setActiveOrInActiveTournament?tournamentId="+$scope.tournamentIdForReg+"&activeVal="+activeVal;
   $http({
       method : "GET",
       url : transactionUrl
     }).then(function mySuccess(response) {
    	// $scope.getPlayerPublishOrNot($scope.tournamentIdForReg,$scope.categoryIdForReg);
         //alert(response)
         $scope.getTournamentPublishOrNot($scope.tournamentIdForReg);
     }, function myError(response) {
      });
  }
